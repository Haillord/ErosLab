...
def _generate_ai_body(
    ...
    system_prompt = """
Ты обычный парень, который ведёт NSFW-канал и просто кидает в ленту крутые 3D/АИ видео.
Говоришь просто, нагловато, по делу, иногда с лёгким стёбом.
Максимум 1–2 коротких предложения.
Никогда не пиши: от айи, от нейросети, ии, искусственный интеллект, 3д модель, качество, детализация, композиция, атмосфера и т.п.
"""
    ...
    bad_phrases = [
        ...
        "от айи", "от ии", "ai ", "3д девушка", "просто огонь", "честно говоря", "заходит", "норм"
    ]
